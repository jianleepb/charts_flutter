...
abstract class Axis<D> extends ImmutableAxis<D> implements LayoutView {
  ...
  void paint(ChartCanvas canvas, double animationPercent) {
    ...
    // 画KM
    var label = (_tickFormatter as NumericTickFormatter).formatValue(0.0);
    if (label == "KM" || label == "S") {
      final textElement =
          label == "S" ? null : graphicsFactory.createTextElement(label);
      tickDrawStrategy.draw(canvas, Tick()..textElement = textElement,
          orientation: axisOrientation,
          axisBounds: _componentBounds,
          drawAreaBounds: _drawAreaBounds,
          isFirst: true,
