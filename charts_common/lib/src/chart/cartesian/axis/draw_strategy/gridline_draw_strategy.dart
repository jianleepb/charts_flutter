...
class GridlineTickDrawStrategy<D> extends BaseTickDrawStrategy<D> {
  ...
  void draw(ChartCanvas canvas, Tick<D> tick,
    ...
    if (tick.value == null) {
      // 空tick
      // 画最左边线
      lineStart = Point(axisBounds.left, drawAreaBounds.top);
      lineEnd = Point(axisBounds.left, axisBounds.top);
      canvas.drawLine(
      ...
      // 画KM
      if (tick.textElement != null) {
        canvas.drawText(tick.textElement..textStyle = labelStyle,
            axisBounds.left, axisBounds.top + 4);
      }
      // 画最右边线
      lineStart = Point(axisBounds.right, drawAreaBounds.top);
      lineEnd = Point(axisBounds.right, axisBounds.top);
      canvas.drawLine(
        points: [lineStart, lineEnd],
        dashPattern: lineStyle.dashPattern,
        fill: lineStyle.color,
    ...
    if (orientation == AxisOrientation.left ||
      ...
      // 底部坐标轴
      if (lineStart.x - axisBounds.left < 10) {
        // 最左边不画
        return;
      } else if (lineStart.x - (axisBounds.left + axisBounds.width) > 0.1) {
        return;
      }
    }
