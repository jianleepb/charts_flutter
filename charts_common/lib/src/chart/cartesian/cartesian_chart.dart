...
import 'axis/spec/axis_spec.dart' show AxisSpec, TextStyleSpec, LineStyleSpec;
...
class NumericCartesianChart extends CartesianChart<num> {
  ...
  void initDomainAxis() {
    ...
    _domainAxis.tickDrawStrategy = GridlineRendererSpec<num>(
            labelStyle: TextStyleSpec(
                fontSize: 6, color: Color.fromHex(code: '#747B81')),
            lineStyle: LineStyleSpec(color: Color.fromHex(code: '#43566C')))
        .createDrawStrategy(context, graphicsFactory);
  }
}
...
abstract class CartesianChart<D> extends BaseChart<D> {
  ...
  set domainAxisSpec(AxisSpec axisSpec) {
    if (_domainAxisSpec != axisSpec) {
      if (_domainAxis != null) {
        axisSpec.configure(_domainAxis, context, graphicsFactory, needReset: false);
      } else {
