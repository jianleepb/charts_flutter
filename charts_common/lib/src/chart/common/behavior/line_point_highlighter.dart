...
class LinePointHighlighter<D> implements ChartBehavior<D> {
  ...
  /// Whether or not to draw datum info through the selected points.
  ///
  /// Defaults to drawing datum info only for the nearest datum.
  final LinePointHighlighterDatumTipType shwoDatumTip;

  /// datum info text style draw through the selected points.
  ///
  /// Defaults is null
  final TextStyleSpec datumTipStyleSpec;
  ...
  LinePointHighlighter(
      ...
      LinePointHighlighterDatumTipType shwoDatumTip,
      List<int> dashPattern,
      bool drawFollowLinesAcrossChart,
      TextStyleSpec datumTipStyleSpec,
      ...
      : selectionModelType = selectionModelType ?? SelectionModelType.info,
        ...
        shwoDatumTip = shwoDatumTip ?? LinePointHighlighterDatumTipType.defalut,
        datumTipStyleSpec = datumTipStyleSpec,
  ...
  void paint(ChartCanvas canvas, double animationPercent) {
    ...
    for (_PointRendererElement<D> pointElement in points) {
      ...
      if (shouldShowVerticalFollowLine &&
        ...
        final selectedDatum =
            chart.getSelectionModel(SelectionModelType.info).selectedDatum;
        final seriesDisplaynames = chart.getSeriesDisplaynames();
        if (!seriesDisplaynames.isEmpty) {
          var labels = [];
          seriesDisplaynames.forEach((displayName) {
            var hasValue = false;

            for (var datum in selectedDatum) {
              if (datum.series.displayName == displayName) {
                labels.add(
                    datum.series.displayName + ': ' + ((displayName == "心率"||displayName == "踏频"||displayName == "功率") ? datum.datum.y.toStringAsFixed(0) : datum.datum.y.toString()));
                hasValue = true;
                break;
              }
            }
          ...
          final textStyle = _getTextStyle(graphicsFactory, datumTipStyleSpec);
          int y = topBound - 30;
          labels.forEach((label) {
            final textElement = graphicsFactory.createTextElement(label)
              ..textStyle = textStyle;
            canvas.drawText(textElement, pointElement.point.x.toInt(), y);
            y += 15;
          });
        }
      }
...
/// Type of value to draw.
enum LinePointHighlighterDatumTipType {
  /// Draw a displayName and value for highlight in the selection.
  defalut,

  /// Draw no datum info.
  none,

  /// Draw a value for highlight in the selection.
  onlyValue,
}
